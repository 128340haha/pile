...
from config import setting                         #见config.py
...
def params_filter(params):
    ...
    for k in ks:
        v = params[k]
        k = smart_str(k, setting['ALIPAY_INPUT_CHARSET'])
        if k not in ('sign','sign_type') and v != '':
            newparams[k] = smart_str(v, setting['ALIPAY_INPUT_CHARSET'])
...
def create_direct_pay_by_user(tn, subject, body, bank, total_fee):
    ...
    params['payment_type']  = '47'       #商品购买，只能选这个
    
    # 获取配置文件
    params['partner']           = setting['ALIPAY_PARTNER']
    #params['seller_id']         = settings.ALIPAY_PARTNER
    params['seller_email']      = setting['ALIPAY_SELLER_EMAIL']
    params['return_url']        = setting['ALIPAY_RETURN_URL']
    params['notify_url']        = setting['ALIPAY_NOTIFY_URL']
    params['_input_charset']    = setting['ALIPAY_INPUT_CHARSET']
    params['show_url']          = ''
    ...
    params['sign'] = build_mysign(prestr, setting['ALIPAY_KEY'], setting['ALIPAY_SIGN_TYPE'])
    params['sign_type'] = setting['ALIPAY_SIGN_TYPE']
...
def notify_verify(post):
    ...
    mysign = build_mysign(prestr, setting['ALIPAY_KEY'], setting['ALIPAY_SIGN_TYPE'])
